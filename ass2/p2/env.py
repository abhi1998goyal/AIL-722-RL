...
class TreasureHunt(gym.Env):
    ...
    def step(self, action):
        ...
        reward = self.reward[self.state]
        # if prev_state==399 or prev_state==299 or prev_state==199 or prev_state==99:
        #     reward=0
    ...
    def visualize_policy_execution(self, policy, path = 'output.gif'):
        ...
        for i in tqdm(range(100)):
           ...
           # print(f'step {self.state} reward {reward}')
            rewards.append(reward)
            images.append(self.render(return_image = True))
        
        #print(f'Visualization reward {np.mean(rewards)}')
