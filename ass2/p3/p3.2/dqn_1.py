...
#from SumTreeCache import CacheData
#from sumtree import SumTree 
import logging
from collections import deque
import numpy as np
#*************Double queue implmentation**************
class CacheData:
    def __init__(self, cache_capacity=20000):
        self.cache_capacity = cache_capacity
        self.data = deque(maxlen=cache_capacity)  
        self.priorities = deque(maxlen=cache_capacity)
...
class DeepQAgent:
    ...
    def __init__(self, env, demo_env,device, cache_size = 20000,tau=0.001,beta_start=0.4):
       ...
       # self.optimizer = torch.optim.Adam(self.qnetwork.parameters(), lr = 0.0005)
        self.optimizer = torch.optim.RMSprop(self.qnetwork.parameters(), lr=0.00005)
    ...
    def visualize_policy(self, itr, path = '/home/RL/ass2/plots/p3.2'):
        ...
        for i, e in enumerate(self.demo_env[:5]):
           ...
           # e.visualize_policy(policy, path_)
            ...
            path_ = os.path.join(path, f'2_TreasureHuntV2_policy_{i}_{itr}.gif')
    ...
    def learn_policy(self, itrs = 50000,gamma=0.99,max_episode_length=100):
        ...
        #for i in range(1, itrs):
            ...
            for j in range(max_episode_length):
                ...
                if len(self.dataset.data) >=64 and len(self.dataset.data)%20==0 :
                    states, actions, next_states, rewards, weights, indices = self.dataset.sample_batch(64, beta=self.beta)
                    states = torch.tensor(states).float().to(self.device)
            ...
            self.eps= max(self.eps_min, 1 * (self.eps_decay ** (i)))
            self.beta = min(1.0, self.beta + 0.001)
            episodes_rewards.append(epi_reward)
           

            if i % 1000 == 0:
                
                policy_rewards=np.mean(episodes_rewards[-100:])
                #demo_values = self.qnetwork.get_all(self.demo_state).max().item()
                rewards = self.validate(100)
                if i%1000==0:
                   self.visualize_policy(i)
                   logging.info(f"loss: {sum(losses)/len(losses)}  validation reward: {rewards} Policy rewards:{policy_rewards} eps: {self.eps} ")
             
                wandb.log({'Validation rewards': rewards, "Episodes": i, "Policy rewards":policy_rewards, "eps": self.eps})
                #logging.info(f"loss: {sum(losses)/len(losses)}  validation reward: {rewards} Policy rewards:{policy_rewards} eps: {self.eps} ")
               # logging.info(f"Saved Checkpoint!")
                ...
                losses = []
                if rewards > best_avg_score :
                    best_avg_score = rewards
                    torch.save(self.qnetwork.state_dict(), '/home/RL/ass2/saved_model/TreasureHuntv2/ckpt_1.pth')
                    print(f"Saved Checkpoint !")
                    logging.info(f"Saved Checkpoint!")

                if rewards>-0.7 or policy_rewards>-0.7:
                   torch.save(self.qnetwork.state_dict(), '/home/RL/ass2/saved_model/TreasureHuntv2/ckpt_final.pth')
...
wandb.init(project="single_01",
               mode="offline",
               name="TreasureHunt_Importance_DQN_Decay_0.9"
               )

device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
logging.basicConfig(filename='/home/RL/ass2/logs/P3.2_training_2.log', level=logging.INFO)
demo_envs = []
for i in range(100):
