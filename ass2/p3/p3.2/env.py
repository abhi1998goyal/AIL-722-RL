...
class TreasureHunt_v2:
    ...
    def check_done(self):
        treasures_collected = np.all(self.state[2] == 0)

        ship_location = np.argwhere(self.state[3] == 1)[0] 
        at_fort = (ship_location[0], ship_location[1]) == (9, 9)

        return treasures_collected or at_fort
