...
class TreasureHunt(gym.Env):
    ...
    def locations_from_state(self, state):
        ...
        treasure_index = state // (self.n*self.n)
        ...
        ship_location = state % (self.n*self.n)
...
class TreasureHuntExtreme:

    def __init__(self, N = 10, num_treasures = 2, num_pirates = 2, num_lands = 10, locations = None):
        ...
        if(locations is None):
            ...
            locations['ship'] = [(0,0)]
            locations['fort'] = [(self.n-1,self.n-1)]

        #make the grid for the pirate and land
        self.land_spatial_state = np.zeros((self.n,self.n))
        for x,y in locations['land']:
            self.land_spatial_state[x,y] = 1
        self.pirate_spatial_state = np.zeros((self.n,self.n))
        for x,y in locations['pirate']:
            self.pirate_spatial_state[x,y] = 1

        #get the model
        self.env = TreasureHunt(locations, self.n)
    ...
    def spatial_to_index(self, state):
        ...
        index = (self.n*self.n)*index + np.argmax(state[3])
        return index 
    ...
    def reset(self):
        ...
        self.env = TreasureHunt(locations, N = self.n)
        self.env.reset()
        self.state = self.index_to_spatial(self.env.state)
        return self.state
    ...
    def index_to_spatial(self, index):
        ...
        spatial_state = np.zeros((4, self.n, self.n))
    ...
    def sample_random_locations(self, num_treasures, num_pirates, num_lands):
        ...
        locations = [] 
        possible_locations = np.arange(1,self.n*self.n -1).tolist()
