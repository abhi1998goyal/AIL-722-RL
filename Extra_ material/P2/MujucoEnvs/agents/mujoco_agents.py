...
class RLAgent(BaseAgent):
    ...
    @torch.no_grad()
    def get_action_from_dist(self, observation: torch.FloatTensor,std_dev=0.1,env=None):
        """
        ...
        with torch.no_grad(): 
             ...
             action_dist = torch.distributions.Normal(mean_action, std_dev)
             sampled_action = action_dist.sample()
             action_low = torch.as_tensor(env.action_space.low, device=ptu.device, dtype=torch.float32)
             action_high = torch.as_tensor(env.action_space.high, device=ptu.device, dtype=torch.float32)
             sampled_action = torch.clamp(sampled_action, action_low, action_high)
    ...
    def load_state_dict(self,state_dict):
        ...
        self.PolicyNet = self.PolicyNet.load(state_dict)
    ...
    def update(self,Trajs):
        ...
        for Traj_i in Trajs:
            ...
            action_dist = torch.distributions.Normal(Pi_theta_at, self.hyperparameters['std_dev_min'])
            log_probs = action_dist.log_prob(Actions).sum(dim=-1)
            loss-=(log_probs*Qit)/(len(States))
        loss=loss/len(Trajs)
        loss.backward()
        self.optimizer.step()   
...
class ActorCriticAgent(RLAgent):
    ...
    def train_iteration(self, env, envsteps_so_far, render=False, itr_num=None, **kwargs):
        ...
        get_action_fn = partial(self.get_action_from_dist,std_dev=std_dev,env=env)
        Trajs = utils.sample_n_trajectories(env, get_action_fn, ntraj=self.hyperparameters['ntraj'], max_length=self.hyperparameters['max_length'], render=False)
