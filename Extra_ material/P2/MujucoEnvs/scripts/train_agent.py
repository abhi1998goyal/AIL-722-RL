...
def setup_agent(args, configs):
    ...
    elif args.exp_name == "AC":
        from agents.mujoco_agents import ActorCriticAgent as Agent
    else:
...
def train_agent(args, configs):
    ...
    for itr in range(configs['num_iteration']):
        ...
        if itr % args.scalar_log_freq == 0:
            ...
            eval_trajs = utils.sample_n_trajectories(env, agent.get_action, 20, max_ep_len)
            ...
            logs.update({'train_loss':train_info['episode_loss'],'train_std_dev':train_info['std_dev']})
...
def main():
    ...
    parser.add_argument("--exp_name", type=str, choices = [ "RL", "AC"], required=True)
